...
def plot_bsm_comp_graph(ax=None, title="BSM Call Price – Computational Graph (DAG)",
                         figsize=(22, 11), save_path=None, dpi=200,
                         node_radius=0.38, font_size=12, edge_lw=1.4,
                         colormap="Set3"):
    """
    Draw a publication-quality computational graph (DAG) for the
    Black-Scholes-Merton call-pricing formula.

    Inputs flow left → right:
        S, K, τ, r, q, σ  ──►  intermediate ops  ──►  C
    ...
    node_radius : float
        Radius of every node circle.
    font_size : float
        Base font size inside nodes.
    edge_lw : float
        Line width of edges / arrows.
    colormap : str
        Matplotlib qualitative colormap used for node colours.
    ...
    def _node_radius(nid):
        ...
        length = len(txt)
        if length > 18:
            return base * 2.2, base * 1.0
        elif length > 10:
            return base * 1.7, base * 0.9
        else:
            return base, base
    ...
    for src, dst, lbl in edges:
        ...
        xs, ys = x0 + ux * (pad_src + 0.05), y0 + uy * (pad_src + 0.05)
        xe, ye = x1 - ux * (pad_dst + 0.05), y1 - uy * (pad_dst + 0.05)
        ...
        if lbl:
            ...
            ax.text(mx, my, lbl, ha="center", va="center",
                    fontsize=font_size - 2, color=text_colour,
                    bbox=dict(boxstyle="round,pad=0.18", fc=bg_colour,
                              ec=legend_edge),
                    zorder=5)
    ...
    for nid, x, y, label, cat in nodes:
        ...
        ellipse = mpatches.FancyBboxPatch(
            ...
            facecolor=fc, linewidth=1.6, zorder=3,
        )
        ax.add_patch(ellipse)


        shadow = mpatches.FancyBboxPatch(
            (x - rx + 0.06, y - ry - 0.06), 2 * rx, 2 * ry,
            boxstyle=f"round,pad=0.0,rounding_size={min(rx, ry) * 0.9}",
            facecolor="#00000015", edgecolor="none", linewidth=0, zorder=2,
        )
        ax.add_patch(shadow)

        ax.text(x, y, label, ha="center", va="center",
                fontsize=font_size if len(label) < 14 else font_size - 2,
                fontweight="bold", color=text_colour, zorder=4)

    y_ann = -1.8
    ax.annotate("", xy=(8, y_ann), xytext=(1, y_ann),
                arrowprops=dict(arrowstyle="-|>", lw=2.2, color=LAB_COLORS[0]))
    ax.text(4.5, y_ann - 0.55, "Forward pass  (evaluate price)",
            ha="center", fontsize=font_size, fontstyle="italic", color=LAB_COLORS[0])

    ax.annotate("", xy=(15, y_ann), xytext=(22, y_ann),
                arrowprops=dict(arrowstyle="-|>", lw=2.2, color=LAB_COLORS[1]))
    ax.text(18.5, y_ann - 0.55, "Reverse pass  (chain-rule → Greeks)",
            ha="center", fontsize=font_size, fontstyle="italic", color=LAB_COLORS[1])
    ...
    lx, ly = 0.5, 10.8
    for i, (cat, name) in enumerate(legend_items):
        rect = mpatches.FancyBboxPatch(
            (lx + i * 4.2, ly), 0.55, 0.55,
            boxstyle="round,pad=0.0,rounding_size=0.2",
            facecolor=cat_colours[cat], edgecolor=edge_colour, lw=1.2, zorder=3,
        )
        ax.add_patch(rect)
        ax.text(lx + i * 4.2 + 0.85, ly + 0.27, name,
                fontsize=font_size - 1, va="center", color=text_colour)
