...
def main() -> int:
    ...
    factors.to_csv(DATA / "fama_french_developed_ex_us_5_factors.csv", index=False)
    momentum.to_csv(DATA / "fama_french_developed_ex_us_momentum.csv", index=False)
    print(f"wrote {DATA / 'fama_french_developed_ex_us_5_factors.csv'} rows={len(factors):,}")
    print(f"wrote {DATA / 'fama_french_developed_ex_us_momentum.csv'} rows={len(momentum):,}")
    return 0
